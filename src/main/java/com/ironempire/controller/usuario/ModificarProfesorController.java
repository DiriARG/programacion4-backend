...
public class ModificarProfesorController {

    private final ModificarUsuarioService modificarUsuarioService;

    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN_GENERAL')")
    public ResponseEntity<UsuarioResponse> modificarProfesor(
            @PathVariable Long id,
            @Valid @RequestBody ModificarUsuarioRequest request) {

        UsuarioResponse response = modificarUsuarioService.modificarProfesor(id, request);
