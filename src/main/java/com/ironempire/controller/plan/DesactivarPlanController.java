...
public class DesactivarPlanController {

    private final GestionarEstadoPlanService gestionarEstadoPlanService;

    @PatchMapping("/{id}/desactivar")
    @PreAuthorize("hasRole('ADMIN_GENERAL')")
    public ResponseEntity<PlanResponse> desactivarPlan(
            @PathVariable Long id) {

        PlanResponse response = gestionarEstadoPlanService.desactivarPlan(id);
